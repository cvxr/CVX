...
%   expressions are both convex and concave, so they can be involved in a
%   difference with any nonlinear expression. Complex affine (or constant)
%   expressions, however, are neither, so they can only be involved in
%   differences with other affine expressions. So, for example, the 
%   following differences are valid:
