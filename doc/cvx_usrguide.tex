...
that adhere to a limited set of construction rules, which
enables them to be analyzed and solved efficiently. The set of 
...
\cvx can solve problems from all of these problem classes,
...
some forms of the power function $x^p$, and 
...
For background on composition rules for convex functions, 
see \cite[\S 3.2.4]{BV:04}.
...
\begin{center}
\begin{tabular}{lll}
	\verb@norm( x, p )@ & $\|x\|_p~~(1<p<+\infty)$ & convex in $x$, nonmonotonic
\end{tabular}
\end{center}
...
since \verb@sum@ is both convex and nondecreasing, and concave and 
nondecreasing.
...
The solvers used in this version of \cvx, SeDuMi or SDPT3, do
...
important; confusing the two operators can have serious numerical consequences.
...
\item For matrices, \verb@p@ must be \verb@1@, \verb@2@, \verb@Inf@, and
\verb@'Fro'@.
\end{itemize}
...
nonnegative. Concave and nondereasing.
\end{itemize}
...
entropy function: \verb@entr(x)=\verb@x.*log(x)@. Concave.
...
\item $\dagger$ \verb@kl_div@, elementwise Kullback-Liebler distance:
\verb@kl_div(x,y)=x.*log(x./y)-x+y@. Convex.
...
is symmetric (if real) or Hermitian (if complex).
Concave.
\item $\dagger$ \verb@logsumexp(x)@, \verb@log(sum(exp(x)))@: the
...
DCPs.
\item \verb@logsumexp_sdp@: a polynomial approximation to \verb@logsumexp@
with global absolute accuracy. This approximation is used in default GP
...
\cvx does allow new convex and concave functions to be added to the
atom library. Indeed, we have steadily added new functions to the library over
...
which we describe in this section. This should be considered a \emph{very}
advanced topic, however---to be attempted only by those who are truly comfortable
with disciplined convex programming and \cvx in its current state.
...
is also possible, and relies on the following basic result of convex
analysis.
...
any case zero for $|x|\geq 1$. However,
...
\begin{equation}
	...
	\end{array}
\end{equation}
Specifically, the optimal value of this simple QP is equal to the Huber
