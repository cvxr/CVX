...
advance requirement. In other words, it is designed to support the formulation and
construction of optimization problems that the user 
intends \emph{from the outset} to be convex.

\index{DCP ruleset}
To that end, disciplined convex programming imposes a limited
...
Our goal in the design of \cvx is to greatly simplify the specification and
solution of convex optimization problems. One indication of the progress we have made
towards this goal is the ease with which some users have encountered the limits
of the software's performance, by creating models that are too large for \cvx to solve.

To some degree, such limits are unavoidable due to a number of factors: the finite
resources of a typical PC, the modest overhead imposed by the MATLAB environment;
the general-purpose design of the underlying numerical solver.
Thus while \cvx should perform
well for medium scale problems and many structured large-scale problems, it may
encounter difficulties when scaling to large problem sets.

Despite this advance warning, we would still greatly appreciate if you would send
...
increased its capacity or performance. In others, we have been able to suggest
ways to find equivalent formulations to problems that \cvx is able to process
more efficiently.
...
\begin{equation}
	y^T(b-Ax) = 0 \quad\Longrightarrow\quad
...
them before beginning to build models. The rules are drawn
...
concave by \cvx.
(This version of \cvx, however, does not yet support the 
entropy function.)
...
primarily by its \emph{curvature}. Four
categories of curvature have been identified:
\emph{constant}, \emph{affine}, \emph{convex}, and \emph{concave}.
For functions $f:\Rn\rightarrow\R$ defined on all $\Rn$,
the categories have the following interpretations:
\begin{equation*}
\begin{array}{lc@{\,}lc}
\text{constant:}    & f(\alpha x + (1-\alpha)y) & = f(x) & \forall x,y\in\Rn,~\alpha\in[0,1]\\
\text{affine:}      & \vdots & =    \alpha f(x) + (1-\alpha) f(y) &  \vdots \\
\text{convex:}      & & \leq \alpha f(x) + (1-\alpha) f(y) \\
\text{concave:}     \quad & & \leq \alpha f(x) + (1-\alpha) f(y) \\
\end{array}
\end{equation*}
Of course, there are significant
overlap in these categories. For example, constant expressions are also affine,
...
constraint is equivalent to two real equality constraints, one for the real parts
and one for the imaginary parts. A word of caution: an equality constraint with
a real side and a complex side has the effect of constraining the imaginary
part of the complex side to be zero.
...
\begin{itemize}
\item a valid constant expression;
\item a declared (non-geometric) variable;
...
geometric counterparts in \S\ref{sec:gp-expressions}), then it is 
rejected by \cvx.
...
Of particular note is that these expression rules forbid \emph{products}
between nonconstant expressions,  with the exception of scalar quadratic forms 
(see \S\ref{sec:quadforms} below). For example, the expression \verb@x * log(x)@ is convex, but violates
the no-product rule, so it is rejected by \cvx. We call this the \emph{no-product rule},
...
so let us address these differences here.  We continue
...
Those who are familiar with the practice of \emph{semidefinite
...
in fact, support geometric programming. For now, one of the authors has developed a novel
