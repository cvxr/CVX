...
This name was chosen to communicate the treatment of convex 
programming as a discipline in which the convexity of 
the problem being formulated is considered an advance requirement.
In other words, \cvx is designed to support the formulation and
...
would contain the string \verb@Solved@.
\end{itemize}
...
Dispciplined convex programming error:
...
But this norm is in the base \cvx library, so to specify and solve
...
\section{\cvx operators, functions, and sets}
\label{s-functions}

\subsection{Basic operators and linear functions}

The standard addition, subtraction, multiplication
and division operators are overloaded to work with \cvx,
when appropriate.  For example, two expressions known to be
concave can be added together, and an expression known to be
convex can be divided by a negative scalar (and the result is
concave).
As another example, a vector affine expression can be multiplied
by a constant matrix (of appropriate dimensions), or added to
another affine expression.
General arrays of expressions can be added or subtracted, provided
the rules of disciplined convex programming are respected.
For example, you can subtract one array of expressions from another
provided the corresponding entries have opposite curvature, so
each element of the result is either the difference of a convex
and concave function, or the difference of a concave and a convex
function.

The usual constructors for arrays work, as 
in \verb@[A B; C D]@, or \verb@X([3,4],:)@.
Matlab functions and operators that are linear have been overloaded
to handle \cvx variables and expressions:
\verb@cumsum@, \verb@diag@,
\verb@dot@ (if one argument is constant), 
\verb@fliplr@, \verb@flipud@, \verb@flipdim@, \verb@horzcat@,
\verb@ipermute@, \verb@kron@, \verb@permute@, \verb@repmat@,
\verb@reshape@,
\verb@rot90@, \verb@sum@, \verb@tril@, \verb@triu@, \verb@vertcat@,
\verb@hankel@, \verb@toeplitz@, \verb@trace@.
When these functions involve summation, the rules of disciplined
convex programming must be followed.  For example, \verb@trace(Z)@
will only work if all entries of the diagonal of the (matrix) 
expression \verb@Z@ are known to be convex, or known to be concave.
...
\item \verb@norm( x, 'largest', k )@, for real and complex vectors.
Convex.
...
\begin{itemize}
\item \verb@lorentz(n)@: $\lorentz^n = 
\Condset{(x,y)\in\R^n\times\R}{\|x\|_2\leq y}$
\item \verb@rotated_lorentz(n)@: $\Condset{(x,y,z)\in\R^n\times\R\times\R}{\|x\|_2\leq yz,~y,z\geq 0}$
\item \verb@complex_lorentz(n):@ $\Condset{(x,y)\in\complex^n\times\R}{\|x\|_2\leq y}$
\item \verb@complex_rotated_lorentz(n)@: $\Condset{(x,y,z)\in\complex^n\times\R\times\R}{\|x\|_2\leq yz,~y,z\geq 0}$
\item \verb@semidefinite(n):@ $\symm^n_+ =
\Condset{X\in\R^{n\times n}}{X=X^T,~X\succeq 0}$
\item \verb@hermitian_semidefinite(n):@ $\Condset{X\in\complex^{n\times n}}{X=X^H,~X\succeq 0}$
%\item XXX \verb@norm_ball(n,p)@: $\Condset{x\in\R^n}{\|x\|_p\leq 1}$. Currently, $p=1,2,\infty$ are supported.
%\item XXX \verb@norm_ball(n,'largest',k)@: $\Condset{x\in\R^n}{\|x\|_{lgst,k}\leq 1}$. See \verb@norm(x,'largest',k)@.
\end{itemize}
