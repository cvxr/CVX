...
This includes a wide variety of functions, such as minimums and
maximums, absolute values, quadratic forms, 
...
The log-sum-exp function $\log (\exp x_1+ \cdots + \exp x_n)$ and the
log of the normal cumulative distribution $\log \Phi(x)$ are
replaced with SDP-compatible approximations
(described in Appendix \ref{sec:gp-approx}).

This version of \cvx adds support for a number of functions that cannot
be exactly represented using via LP, SOCP, or SDP, including 
log, exp, entropy, and Kullback-Leibler divergence.
These problems are handled by solving a sequence (typically just a handful)
of SDPs, and yield the solution to the full accuracy of the core solver.
On the other hand, this successive approximation technique can be 
\emph{substantially slower}
than if the core solver directly handled such functions.
We anticipate a large speedup once a
solver with native support for such functions is available;
...
(\texttt{mcgrant@stanford.edu}, \texttt{boyd@stanford.edu}) with 
your comments. If
you discover what you think is a bug, 
...
so it can be used later in two different constraints. We call \verb@z@ an
\emph{expression holder} to differentiate it from a formally declared \verb@cvx@
variable. For more on the critical
...
Matlab variable. Unfortunately, a somewhat technical detail of the Matlab object
...
\verb@1@, and whose subsequent values are convex \cvx expressions. But if you
try this in a \cvx model, Matlab will give you a rather cryptic error:
\begin{code}
...
a numeric array; and Matlab will not permit \cvx objects to be inserted into numeric
arrays. 

The solution is to explicitly \emph{declare} \verb@x@ to be an 
expression holder assigning values to it. We have provided keywords \verb@expression@
...
are quite significant. A \verb@variable@ object holds a unique set of optimization
variables that were created for the problem when the object was created. An
\verb@expression@ object, on the other hand, is initialized to zero, and has
little use until values are assigned to it.
...
(It can be expressed as \verb@pow_p(x,3/2)@, however.)
...
In \cvx, functions are categorized in two ``dimensions'': \emph{curvature}
...
\begin{center}
\begin{tabular}{lll}
	\verb@norm( x, p )@ & $\|x\|_p~~(1<p)$ & convex in $x$, nonmonotonic
\end{tabular}
\end{center}
...
the maximum function is convex and nondecreasing, so it can accept
any convex expressions as arguments.
...
covnex and concave (since it is affine), and nondecreasing 
is each argument.  Therefore the expressions
\begin{code}
	sum( square( x ) )
	sum( sqrt( x ) )
\end{code}
...
so some expressions which are obviously convex or concave will fail to
...
\end{code}
where \verb@x@ is a variable, and \verb@A@ and \verb@b@ are a constant 
matrix and vector of appropriate size.
This expression applies to each element of \verb@A*x+b@ the function
$z \mapsto (z^2+1)^2 = z^4+2z^2+1$, which is convex,
so this expression is elementwise convex. 
But
...
\begin{code}
	square( square( A * x + b ) - 1 )
\end{code}
is not convex. The problem here is that \verb@square@ is not monotonic,
so the composition rules cannot be applied.

Examination of the two expressions above reveals a key difference:
in the first, the argument to the outer \verb@square@ is nonnegative;
in the second, it is not.
In the first case, the composition rule could in principle be 
applied since \verb@square@ \emph{is} nondecreasing, as long as its 
argument is nonnegative.
If \cvx knew that the argument to the outer \verb@square@ function,
\ie, \verb@square(A*x+b)+1@, were nonnegative, it
could conclude that the first composition is convex by applying a
modified form of the first nonlinear composition rule.
For now, however, \cvx does not keep track of the sign of expressions,
so it cannot make such a conclusion.
Instead, \cvx requires that outer functions be 
\emph{globally} monotonic.

There are several ways to modify this example so 
that it complies with the ruleset.
One way is to write
\begin{code}
	( A * x + b ).^4 + 2*( A * x + b ).^2 + ( A * x + b )
\end{code}
which \cvx recognizes as convex, since
\cvx allows positive even integer powers using the \verb@.^@ operator.
(Note that the same technique, applied to the second example, will fail,
since the second term is \verb@-2*(A*x+b).^2@, which is concave.)
...
\begin{equation*}
	f(x) \triangleq (\max\{x,0\})^2 = 
\Cases{ x^2 & x \geq 0 \\ 0 & x \leq 0. }
\end{equation*}
Obviously, \verb@square@ and \verb@square_pos@ coincide when their
arguments are nonnegative. 
But \verb@square_pos@ is \emph{globally} nondecreasing, so it
can be used as the outer function in a nonlinear composition. 
Thus, the expression
\begin{code}
	square_pos( square( A * x + b ) + 1 )
\end{code}
is mathematically equivalent to the rejected version, but satisfies the DCP
ruleset and is therefore accepted by \cvx.
...
\begin{code}
	square_of_square_plus_one( A * x + b )
\end{code}
...
using incomplete specifications, both of we will illustrated using
...
approximation method which allows \cvx to solve GPs and related
problems. The method converts GPs to SDPs, and the approximation error 
can be bounded in advance.
...
\subsection{MEX file compilaton}
\label{sec:othersys}
...
\item Functions marked with a dagger ($\dagger$) are not supported natively by
      the solvers that \cvx uses. We have developed a novel successive approximation
      method for solving models involving these functions. The same precision
      is achieved, but multiple calls to the solver are made. These functions
      are currently \emph{disabled by default}, and must be explcitly enabled
      before your model is constructed. See \S\ref{sec:succ-approx} for details.
...
one-argument version \verb@norm(x)@ computes the 2-norm for vectors and induced
2-norm for matrices. The two-argument version \verb@norm(x,p)@ is supported as follows:
\begin{itemize}
...
be scalars. Only those values of \verb@p@ which can reasonably and unambiguously
interpreted as convex or concave are accepted:
\begin{itemize}
\item $\verb@p@=0$. Constant; that is, \verb@x.^p@ is identically 1.
\item $0<\verb@p@<1$. Concave. \verb@x@ must be concave (or affine), and is
      implicitly constrained to be nonnegative.
\item $\verb@p@=1$. Affine; that is, \verb@x.^p@ is equivalent to \verb@x@.
\item $\verb@p@\in\{2,4,6,8,...\}$. Convex. \verb@x@ must be affine.
\item $\verb@p@>1$, $\verb@p@\not\in\{2,3,4,5,...\}$. Convex. \verb@x@ must be affine,
and is implicitly constrained to be nonnegative.
\end{itemize}
Negative and odd integral values of \verb@p@ are not permitted, but functions
...
constraints are not met. Concave.
\item $\star$ \verb@log_norm_cdf(x)@: logarithm of cumulative distribution
...
DCPs.
\item \verb@log_sum_exp_sdp@: a polynomial approximation to the log-sum-exp
...
that our approximate is accurate enough to represent the orignal model.
