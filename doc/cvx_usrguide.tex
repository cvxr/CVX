...
\begin{itemize}
	...
	\item \verb@Overdetermined@: The presolver has determined that the problem
	      ...
	      matrix of the equality constraints is singular. Such problems are often,
	      but not necessarily, infeasible. Unfortunately, the underlying numerical
	      solvers generally cannot handle such problems, so a precise conclusion
	      cannot be reached.
\end{itemize}
This last status message \verb@Overdetermined@ commonly occurs when structure
...
is that the unnamed variable returned by \verb@semidefinite(n)@ is \emph{symmetric}, but
