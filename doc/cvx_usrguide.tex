...
\item Inequality constraints involving non-square matrices are \emph{disallowed};
      attempting to use them causes an error.
...
\item Note that LMI constraints enforce symmetry (real or Hermitian, as
      ...
      of symmetric matrices, this is reasonably straightforward.
...
For example, inside a \cvx model in SDP mode,
the expression \verb@X >= 0@ above constrains \verb@X@
to be positive semidefinite. But after the model has been solved
and \verb@X@ has been replaced with a numeric value,
the expression \verb@X >= 0@ will test for the \emph{elementwise}
nonnegativity of \verb@X@.

Therefore, we ask that
users explicity request SDP mode when they intend to use it,
so that they acknowledge and accept this inconsistency in advance.
We hope that you will agree that this represents only a slight
inconvenience even to those who use it most frequently.
...
Concave and increasing.
\item \verb@det_root2n@: $2n$-th root of the determinant of a semidefinite matrix,
$(\det X)^{1/2n}$. When used inside a \cvx specification, \verb@det_root2n@ constrains
the matrix to be symmetric (if real) or Hermitian (if complex) and positive semidefinite. 
When used with numerical arguments, \verb@det_root2n@ returns \verb@-Inf@ if these
