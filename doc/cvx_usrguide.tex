...
\index{cvx_solver_exp@\texttt{cvx\_solver\_exp}}
...
One group of problems for which SDPT3 is generally more reliable
are those for which \cvx employs the successive approximation described in
Appendix~\ref{sec:succ-approx}: that is, 
geometric programs and problems involving logarithms, exponentials,
and entropy functions. This is due to technical issues with our heuristic,
not due to any problems with SeDuMI. For such problems, SDPT3
is the default. You can use SeDuMi instead by typing
\begin{code2}
	cvx_solver_exp sedumi
\end{code2}
at the command line. As with \verb@cvx_solver@, \verb@cvx_solver_exp@
can be issued within
a model to select a different solver for that model only; or it can
be issued outside of a model to change the default for all
successive approximation models.
