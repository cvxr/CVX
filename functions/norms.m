...
if cvx_isconstant( x ),
    x = cvx_constant( x );
    switch nargin,
        case 1, cvx_optval = norms( x );
        case 2, cvx_optval = norms( x, p );
        case 3, cvx_optval = norms( x, p, dim );
    end
    return
end
sx = size( x );
...
if nargin < 2 | isempty( p ),
    p = 2;
elseif ~isnumeric( p ) | any( size( p ) ~= 1 ) | ~isreal( p ),
...
if length( sx ) < dim | sx( dim ) == 1,
    ...
    switch p,
        ...
        otherwise,
            if p < 1,
                error( sprintf( 'Disciplined convex programming error:\n   The %g-norm is not a true, convex norm.', p ) );
            else,
